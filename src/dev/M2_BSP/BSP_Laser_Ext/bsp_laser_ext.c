...
mcp4902_dev_t laser_ext_dac_dev = {
    .cs = &laser_int_dac_cs,
    .latch = &laser_int_dac_latch,
};
...
void bsp_laser_ext_init(void) {
    ...
    adg1414_chain_qspi_init(&laser_ext_dev, &qspi, laser_ext_dev.cs, laser_ext_dev.num_of_sw);
    mcp4902_dev_qspi_init(&laser_ext_dac_dev, &qspi, laser_ext_dac_dev.cs, laser_ext_dac_dev.latch);
}

void bsp_laser_ext_set_dac(uint8_t code) {    
    mcp4902_set_dac(&laser_ext_dac_dev, MCP4902_CHA, code);
}

uint8_t bsp_laser_ext_get_dac(void) {
    return laser_ext_dac_dev.dac_channel[MCP4902_CHA];
}
...
void bsp_laser_ext_sw_off(uint8_t channel) {
    adg1414_chain_sw_off(&laser_ext_dev, channel);
}

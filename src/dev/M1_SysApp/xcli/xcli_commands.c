...
static void BootManager_SystemDeInit(void) {
    ...
    /* ---- 3. Stop/Disable DMA channels ---- */
    /* Abort any active XDMAC transfers */
    XDMAC_ChannelDisable(XDMAC_CHANNEL_0);
    XDMAC_ChannelDisable(XDMAC_CHANNEL_1);

    /* ---- 4. Flush UART TX before reset ---- */
    /* Wait for UART0 TX empty */
    while (!(UART0_REGS->UART_SR & UART_SR_TXEMPTY_Msk));
    /* Wait for UART2 TX empty */
    while (!(UART2_REGS->UART_SR & UART_SR_TXEMPTY_Msk));
    ...
    /* ---- 7. Disable AFEC ---- */
    AFEC0_REGS->AFEC_CHDR = 0xFFFFFFFFU;
    AFEC1_REGS->AFEC_CHDR = 0xFFFFFFFFU;

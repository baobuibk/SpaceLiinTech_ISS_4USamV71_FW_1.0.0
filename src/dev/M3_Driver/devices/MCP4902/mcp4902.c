...
static int mcp4902_write_frame(mcp4902_dev_t *dev, uint16_t frame) {
    ...
    if (dev->bus == MCP_BUS_SPI) {
        ...
        do_set(dev->cs);
        mcp4902_latch_pulse(dev);
    } else if (dev->bus == MCP_BUS_DMA_SPI) {
        SPI1_REGS->SPI_CSR[0] |= SPI_CSR_BITS_8_BIT;

        dma_spi1_obj = DMA_SPI1_INT_LASER;
...
static uint16_t mcp4902_build_frame(uint8_t channel, uint8_t code, uint8_t gain1x, uint8_t enable) {
    ...
    if (channel == MCP4902_CHB)
        f |= (1u << MCP4902_AB_BIT);
    if (gain1x)
        f |= (1u << MCP4902_GA_BIT); /* 1 = 1x */
    if (enable)
        f |= (1u << MCP4902_SHDN_BIT); /* 1 = bật */

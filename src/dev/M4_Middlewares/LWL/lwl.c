...
#define LWL_BUF_THRESHOLD 		(LWL_BUF_SIZE - 2)
...
void lwl_buffer_full_notify() {
    if (psram_store_log_queue == NULL) return;

    void *p_full_buf = (void *) lwl.lwl_data_buf[lwl.lwl_full_buf_index].p_buf;
    xQueueSend(psram_store_log_queue, &p_full_buf, 0);

    psram_full_object_t object = FULL_LOG_DATA;
    xQueueSend(psram_notify_full_queue, &object, 0);
}

void lwl_clear_notification() {
